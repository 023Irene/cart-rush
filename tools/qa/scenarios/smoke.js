...
async function main() {
  ...
  // Монеты считаются от суммарной ценности БЕЗ умножения на количество (этап 8.4):
  // именно это отвязывает магазин от рекорда. Если формулы когда-нибудь сольют
  // обратно в одну, счёт останется верным, а монеты подскочат вчетверо
  const expectedCoins = 25 * 4 * 2;
  ok('сдача начислила монеты', unloaded.coins === expectedCoins,
    `монет ${unloaded.coins}, ожидалось ${expectedCoins}`);

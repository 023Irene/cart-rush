...
## Звуки, которых здесь нет

**Взрыв бомбы (этап 5) отдельным файлом не добавлялся.** Играет тот же `error.ogg`, опущенный
на 700 центов (`CONFIG.audio.blastDetune`): ниже он звучит длиннее и глуше и на короткий удар
уже не похож. Подбор бустера — `deliver.ogg`, поднятый на 400 центов.

Решение временное и проверяется ушами. Если взрыв не читается, брать отдельный сэмпл из пака
**Kenney — Sci-Fi Sounds** (тоже CC0) и дописывать таблицу выше.
